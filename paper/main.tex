...
%   neurips_2026.sty           - official 2026 style (loaded below)
%   checklist.tex              - mandatory NeurIPS Paper Checklist
%   neurips_2026_reference.tex - the official template, kept for reference only
%   references.bib             - bib database
% =============================================================================
...
% Submission (default = main track, anonymized double-blind reviewing).
\usepackage{neurips_2026}

% Other tracks (uncomment exactly one if applicable):
%   \usepackage[main]{neurips_2026}              % Main Track (same as default)
%   \usepackage[position]{neurips_2026}          % Position Paper Track
%   \usepackage[eandd]{neurips_2026}             % Evaluations & Datasets Track
%   \usepackage[eandd, nonanonymous]{neurips_2026}  % E&D single-blind opt-in
...
% Camera-ready - swap to this only after acceptance, e.g.:
...
Task arithmetic~\cite{ilharcoEditingModelsTask2022} treats the difference
between fine-tuned and pretrained weights as a composable ``task vector''
that can be added or negated to edit model behavior. Tangent task
arithmetic~\cite{ortiz-jimenezTaskArithmeticTangent2023} extends this to the
linearized regime, and weight-space averaging methods such as model
soups~\cite{wortsmanModelSoupsAveraging2022} demonstrate that simple linear
combinations of fine-tunes can match or surpass individual checkpoints.
% TODO: Frame: task vectors are scalar-weighted; we generalize to learned
% block-anisotropic weights and predict them.
% TODO(missing-bib): Yadav et al. TIES-Merging (NeurIPS 2023).

\subsection{Anisotropic Scaling and aTLAS}
Our work directly builds on aTLAS~\cite{zhangKnowledgeCompositionUsing2024},
which introduces anisotropic task-level scaling: rather than weighting an
entire task vector by a single scalar, aTLAS learns a separate coefficient
per parameter block, exposing the low intrinsic dimensionality of pretrained
models for cheap composition. We adopt the aTLAS coefficient parameterization
unchanged and ask a different question: instead of fitting these coefficients
per task by gradient descent, can a single amortized model predict them from
a task description and a handful of support images?
% TODO: Mention partition variant (aTLAS x K) and linearized variant when
% setting up the experimental sweep.

\subsection{Hypernetworks for Adaptation}
% TODO: Distinguish: prior hypernet work predicts weight matrices/LoRA; we
% predict task-vector coefficients - far smaller output space, much cheaper.
% TODO(missing-bib): Ha et al. HyperNetworks (ICLR 2017); any HyperLoRA work.

\subsection{Few-Shot and Zero-Shot Adaptation of CLIP}
The dominant zero- and few-shot baselines for CLIP-class
models~\cite{radfordLearningTransferableVisual2021} either tune text prompts
(CoOp~\cite{zhouLearningPromptVisionLanguage2022},
MaPLe~\cite{khattakMaPLeMultimodalPrompt2023}) or interpolate between
fine-tuned and pretrained weights
(WiSE-FT~\cite{wortsmanRobustFinetuningZeroshot2022}). We compare against
these but operate on a different primitive --- aTLAS coefficients --- and
predict that primitive in a forward pass rather than learning it per task.
% TODO(missing-bib): CoCoOp (Zhou et al. 2022); Tip-Adapter (Zhang et al. 2022);
% LP++ (Huang et al. 2024).

\subsection{Meta-Learning}
% TODO: Brief; our recipe is closer to amortized inference than
% gradient-based meta-learning.
% TODO(missing-bib): Finn et al. MAML (ICML 2017); Snell et al. ProtoNet
% (NeurIPS 2017).
...
We follow the standard CLIP~\cite{radfordLearningTransferableVisual2021}
setup with a pretrained image encoder $f_{\theta_0}$ and per-task
fine-tunes $f_{\theta_t}$. The task vector $\tau_t = \theta_t - \theta_0$
acts as the unit of composition~\cite{ilharcoEditingModelsTask2022}.
% TODO: expand notation as needed once method section is drafted.

\subsection{Anisotropic Task-Level Scaling (aTLAS)}
The anisotropic task-level scaling formulation we adopt is due to
Zhang~et~al.~\cite{zhangKnowledgeCompositionUsing2024}. Briefly, parameters
are partitioned into blocks $\theta = (\theta^{(1)}, \ldots, \theta^{(B)})$,
and $T$ task vectors are composed with per-block coefficients
$\alpha \in \mathbb{R}^{T \times B}$:
\[
\theta_{\text{new}} = \theta_0 + \sum_{t=1}^{T} \sum_{b=1}^{B}
\alpha_{t,b}\, \tau_t^{(b)}.
\]
In the original formulation $\alpha$ is fit per target task via gradient
descent on labeled adaptation data; our contribution is to amortize this
fit so that $\alpha$ is predicted in a single forward pass from a task
description and (optionally) a few support images.
...
% References
% =============================================================================
% Bibliography source: NeurIPS26.bib (Better-BibTeX export from Zotero).
%
% Confirmed citation keys for must-cite work (use these directly in \cite{}):
%   - CLIP (Radford et al. 2021)            radfordLearningTransferableVisual2021
%   - aTLAS (Zhang et al. 2024)             zhangKnowledgeCompositionUsing2024
%   - Task arithmetic (Ilharco et al.)      ilharcoEditingModelsTask2022
%   - Tangent task arithmetic (Ortiz-J.)    ortiz-jimenezTaskArithmeticTangent2023
%   - WiSE-FT (Wortsman et al. 2022)        wortsmanRobustFinetuningZeroshot2022
%   - Model Soups (Wortsman et al. 2022)    wortsmanModelSoupsAveraging2022
%   - CoOp (Zhou et al. 2022)               zhouLearningPromptVisionLanguage2022
%   - MaPLe (Khattak et al. 2023)           khattakMaPLeMultimodalPrompt2023
%   - Stable Diffusion (Rombach et al.)     rombachHighResolutionImageSynthesis2022
%   - DALL-E (Ramesh et al. 2021)           rameshZeroShotTexttoImageGeneration2021
%
% TODO(missing-refs): the following were NOT found in the Zotero export and
% need to be added manually before submission. Without them the related-work
% and baseline-comparison sections will be incomplete:
%   - Ha, Dai, Le. HyperNetworks. ICLR 2017.
%   - Zhang et al. Tip-Adapter. ECCV 2022.
%   - Huang et al. LP++ (CVPR 2024 best-fit).
%   - Yadav et al. TIES-Merging. NeurIPS 2023.
%   - Zhou et al. Conditional Prompt Learning (CoCoOp). CVPR 2022.
%   - Finn et al. MAML. ICML 2017.
%   - Snell et al. Prototypical Networks. NeurIPS 2017.
%   - Cherti et al. OpenCLIP / reproducible scaling laws. CVPR 2023.
\bibliographystyle{plainnat}
\bibliography{NeurIPS26}
...
% =============================================================================
% The checklist file is the official NeurIPS 2026 template, with answers stubbed
% as \answerTODO{}. Fill in every \answerTODO and \justificationTODO before
% submission. Do this LAST, but do NOT skip it. The checklist does NOT count
% against the 9-page main-content limit.
