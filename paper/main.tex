...
%   \cite{ilharcoEditingModelsTask2022}        % task arithmetic
%   \cite{ortiz-jimenezTaskArithmeticTangent2023}  % tangent task arithmetic
%   \cite{wortsmanModelSoupsAveraging2022}     % model soups
% TODO(missing-bib): Yadav et al. TIES-Merging (NeurIPS 2023).

\subsection{Anisotropic Scaling and aTLAS}
% TODO: Briefly recap the contribution and clarify what is and is not borrowed.
% Block coefficients, partition variant (aTLAS x K), linearized variant.
%   \cite{zhangKnowledgeCompositionUsing2024}  % aTLAS predecessor (NeurIPS '24)
...
% TODO: We compare to these. Note that these methods either learn prompts or
% adapters; we learn a different primitive (task-vector coefficients).
%   \cite{radfordLearningTransferableVisual2021}    % CLIP (zero-shot baseline)
%   \cite{zhouLearningPromptVisionLanguage2022}     % CoOp
%   \cite{khattakMaPLeMultimodalPrompt2023}         % MaPLe (multi-modal prompt)
%   \cite{wortsmanRobustFinetuningZeroshot2022}     % WiSE-FT
...
%   \cite{radfordLearningTransferableVisual2021}    % CLIP
%   \cite{ilharcoEditingModelsTask2022}             % task vectors
...
%   \cite{zhangKnowledgeCompositionUsing2024}        % aTLAS
